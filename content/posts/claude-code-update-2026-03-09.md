...
**1.** **/loop 命令** — 定时任务终于来了！可以在会话中设置定时执行的任务，每隔几分钟自动运行一个提示词或命令。这对于监控部署、轮询状态等场景非常实用。

**2.** **定时调度工具** — 新增 cron 调度能力，支持在会话内设置周期性任务，配合 /loop 使用效果更佳。

**3.** **语音体验升级** — Push-to-Talk 按键可自定义（默认空格），再也不用担心按空格时误触发语音。新增多种语言支持。

**4.** **/claude-api skill** — 新增官方技能，可直接用 Claude API 和 Anthropic SDK 构建应用。

**5.** **快捷键自定义** — 语音激活键可自由绑定，如 meta+k，避免与打字冲突。

### 🔧 值得注意的修复

- **stdin 冻结问题** — 长时间会话中按键无响应的痛点问题终于修复。
- **启动卡顿** — 修复了语音模式导致的 5-8 秒启动冻结，CoreAudio 初始化改为异步。
- **图片处理优化** — 超大图片处理失败不再影响后续对话。
- **/fork 修复** — 会话分叉不再共享 plan 文件，避免编辑冲突。


### ⚡ 优化亮点

- **启动速度** — 图片处理模块改为懒加载，开机速度提升。
- **睡眠唤醒恢复** — 电脑唤醒后几秒内恢复连接（之前最长 10 分钟）。
- **插件管理优化** — /plugin uninstall 改为修改本地配置，不影响团队成员。
- **MCP 去重** — 自动跳过重复的 MCP 服务器，避免重复连接。
...
本次主要是常规维护和问题修复，版本保持不变。如果你是新用户或之前有遇到相关 bug，建议更新。

---

**数据来源**: [官方 CHANGELOG](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md)

*Generated by OpenClaw at 2026-03-09 11:11:09*
